...
export function ThoughtDetailPanel({
  ...
  const resolved = useMemo(() => {
    ...
    const index = track.nodes.findIndex((node) => node.id === selected.nodeId)
    if (index < 0) return null
    return { track, node: track.nodes[index] }
  }, [selected, tracks])
  ...
  useLayoutEffect(() => {
    ...
    const target = hero.offsetTop - scroller.clientHeight / 2 + hero.offsetHeight / 2
  ...
  return (
    ...
    >
          ...
          {track.nodes.map((node) => {
            ...
            return (
              <NodeCard
                ...
                onClick={() => {
                  if (!hero) onSelectNode(track.id, node.id)
                }}
              />
            )
          })}
      ...
      {composerEnabled ? (
        <div className="shrink-0 border-t border-white/[0.05] px-5 py-3" style={{ backgroundColor: "#0a0a0c" }}>
          <div className="flex items-end gap-2 rounded-2xl border border-white/[0.08] px-3 py-2">
            <Textarea
              value={input}
...
const NodeCard = forwardRef<HTMLDivElement, NodeCardProps>(function NodeCard(
  ...
  if (hero) {
    return (
      ...
      >
        <div className="text-[11px]" style={{ color: "rgba(237,230,212,0.45)", letterSpacing: "0.06em" }}>
          {time}
        </div>
        <div
          className="mt-2 text-[16.5px] [overflow-wrap:anywhere]"
          style={{ color: "#EDE6D4", lineHeight: 1.65 }}
        >
          {node.questionText}
        </div>
        ...
        {imageSrc ? (
          <div className="mt-4 overflow-hidden rounded-[14px] border border-white/[0.04]">
            ...
            <img src={imageSrc} alt="" className="block h-auto max-h-[260px] w-full object-cover" />
          </div>
        ) : null}

        {note ? (
          <p className="mt-4 text-[13px] [overflow-wrap:anywhere]" style={{ color: "rgba(237,230,212,0.55)", lineHeight: 1.7 }}>
            {note}
          </p>
        ) : null}

        {answer ? (
          <div className="mt-4 rounded-[14px] px-4 py-3" style={{ backgroundColor: "rgba(255,255,255,0.025)" }}>
            <div className="text-[10.5px]" style={{ color: "rgba(237,230,212,0.4)", letterSpacing: "0.08em" }}>
              我的回应
            </div>
            <p
  ...
  return (
    ...
    >
      <div className="text-[10.5px]" style={{ color: "rgba(237,230,212,0.32)", letterSpacing: "0.06em" }}>
        {time}
      </div>
      <div
      ...
      {imageSrc || answer ? (
        <div className="mt-1 text-[10.5px]" style={{ color: "rgba(237,230,212,0.28)", letterSpacing: "0.04em" }}>
          {[imageSrc ? "图片" : null, answer ? "已记录回应" : null].filter(Boolean).join(" · ")}
        </div>
      ) : null}
    </div>
  )
