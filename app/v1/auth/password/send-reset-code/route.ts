  ...
  async (request: NextRequest) => {
    ...
    await updateDb((db) => {
      ...
      const active = db.email_verification_codes.find(
        (item) =>
          item.email === email && item.purpose === "reset_password" && !item.consumed_at && new Date(item.expires_at).getTime() > now
      );
    ...
    if (!canSend) return errorJson(409, "请稍后再试或邮箱不存在");

    const code = generateEmailVerificationCode();
    try {
      await sendPasswordResetVerificationCode(email, code);
    } catch {
